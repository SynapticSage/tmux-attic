#!/usr/bin/env bash
# Toggle @recon-ignore on the current tmux WINDOW. Window-scope
# ignores cascade to every pane in the window via tmux's standard
# inheritance chain (pane -> window -> session -> global), so marking
# a window silences every agent inside it in one keystroke — both in
# the recon cycle rotation and in per-window badge statistics.
#
# Pane-scope granularity is deliberately not a binding: the window is
# the natural unit users think about, and the prefix + I picker
# covers the session/window cases for non-focused targets.
#
